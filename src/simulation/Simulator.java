...
public class Simulator implements WorldListener {
	...
	public void nextCycle()
			throws CitizenAlreadyDeadException, BuildingAlreadyCollapsedException, CannotTreatException {
	...
	private void handleFire(Disaster d) throws CitizenAlreadyDeadException, BuildingAlreadyCollapsedException {
		...
		} catch (BuildingAlreadyCollapsedException b1) {
	...
	public void finished() {
		for (int i = 0; i < emergencyUnits.size(); i++) {
			...
			if (emergencyUnits.get(i) instanceof Evacuator) {
				Evacuator s = (Evacuator) emergencyUnits.get(i);
				if (s.getState() == UnitState.IDLE)
					s.free = true;
			...
			{
				Ambulance a = (Ambulance) emergencyUnits.get(i);
			...
			if (emergencyUnits.get(i) instanceof DiseaseControlUnit) {
				DiseaseControlUnit d = (DiseaseControlUnit) emergencyUnits.get(i);
				if (d.getState() == UnitState.IDLE) {
					d.free = true;
				} else
					d.free = false;
			}
			if (emergencyUnits.get(i) instanceof GasControlUnit) {
				GasControlUnit g = (GasControlUnit) emergencyUnits.get(i);
				if (g.getState() == UnitState.IDLE) {
					g.free = true;
				} else
					g.free = false;
			}
			if (emergencyUnits.get(i) instanceof FireTruck) {
				FireTruck f = (FireTruck) emergencyUnits.get(i);
				if (f.getState() == UnitState.IDLE) {
					f.free = true;
				} else
					f.free = false;
			}
	...
	/*
	 * public String getInfo() { String x = ""; for(int i = 0 ; i <
	 * emergencyUnits.size() ; i++) { Unit u = emergencyUnits.get(i); if(u
	 * instanceof Evacuator) { if(((Evacuator)u).getState()== UnitState.RESPONDING
	 * || ((Evacuator)u).getState()== UnitState.TREATING) { x = u.getLocation()+"";
	 * 
	 * } } } return x; }
	 */
	...
	public String Update() {
		String x = "";
		for (int i = 0; i < executedDisasters.size(); i++) {
			Disaster a = executedDisasters.get(i);
